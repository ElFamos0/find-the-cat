...
int get_file_by_mime(char * value, path_list *pl){
    int i = 0;
    //printf("je commence la recherche\n");
    while (i < pl->ptr){
        int incr = 1;
        ...
        //printf("l'extension est %s\n",ext);
        if (ext == NULL){
            //printf("No extension found for %s\n", path);
            delete_path(pl,path);
            free(path);
            continue;
        }
        int j = 0;
        int found = 0;
        while(mime_list[j][0] != NULL){
            if (strcmp(mime_list[j][0],ext) == 0){
                //printf("le mime du fichier %s est %s\n",path,mime_list[j][1]);
                if (strncmp(mime_list[j][1], value,strlen(value)) == 0){
